...
num_epochs = 50
batch_size = 4   # Reduced for memory constraints - 80GB GPU with other processes
lr = 1e-4
seed = 42
CLASS_IDS = (0, 1, 2, 3)
...
class CheckpointManager:
    ...
    def save_checkpoint(self, model, optimizer, epoch, metrics, filename):
        """Save checkpoint with size validation"""
    ...
    def save_weights_only(self, model, epoch, metrics, filename, dtype=torch.float16):
        """Save weights-only checkpoint, optionally in reduced precision to cut size."""
        file_path = os.path.join(self.checkpoint_dir, filename)
        ...
        checkpoint_data = {
            ...
            'optimizer_state_dict': None,  # not included
            'metrics': metrics,
...
class DiceCrossEntropyAFWLoss(nn.Module):
    ...
    def forward(self, preds, target):
        ...
        afw_loss = torch.tensor(0.0, device=preds.device)
...
def train_model(train_loader, val_loader):
    ...
    if os.path.exists(weights_pt_path):
        weights_tensor = torch.load(weights_pt_path, map_location=device)
    ...
    # Fallback: pick most recent epoch checkpoint (with optimizer state)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for images, masks in tqdm(train_loader, desc=f"[Epoch {epoch+1}/{num_epochs}] Training"):
            ...
            with torch.amp.autocast(device_type='cuda', enabled=use_amp):
                ...
                # -------------------------------
                # Add AFW entropy regularization
                # -------------------------------
                lambda_afw = 0.01  # Used to scale AFW_loss to fall within the values of the ce_loss & dice_loss. Adjusted as needed
        ...
        if avg_val_loss < best_val_loss:
            ...
            # Also save a compact weights-only checkpoint for inference
            ...
            # And update a stable small file path for downstream scripts
            checkpoint_manager.save_weights_only(
                model, epoch, epoch_record,
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, _ = get_dataloaders(
        ...
        val_ratio=0.15,
        augment=True,
        num_workers=0, # Prevent multiprocessing freeze
        pin_memory=False # Prevent GPU memory leak
    )
