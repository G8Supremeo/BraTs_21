...
class BraTSDataset(Dataset):
    ...
    def _compute_dataset_statistics(self):
        """Compute dataset-level statistics for analysis"""
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        try:
            ...
            if BraTSDataset.printed_count < 4:
                labels, counts = np.unique(seg_np, return_counts=True)
                logger.info(f"[Sanity Check {BraTSDataset.printed_count+1}] Sample {idx} Label Voxel Counts: {dict(zip(labels, counts))}")
...
def get_dataloaders(data_dir, batch_size, train_ratio=0.7, val_ratio=0.15, augment=False, num_workers=2, pin_memory=True):
    """
    ...
    # Create dataset
    try:
        full_dataset = BraTSDataset(data_dir, augment=augment)
